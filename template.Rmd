...
alcohol_consump <- read_csv("violence_factors/alcohol_consumption_oecd.csv") %>% 
  filter(TIME == "2019", FREQUENCY == "A") %>% 
  select("LOCATION", "Value") %>% 
  rename("location" = "LOCATION", "alcohol_consump" = "Value")

# Read child marriage
child_marriage <- read_csv("violence_factors/child_marriage_rate.csv")  %>% 
  filter(TIME == "2019", FREQUENCY == "A", SUBJECT == "EARMARRIAGE") %>% 
  select("LOCATION", "Value") %>% 
  rename("location" = "LOCATION", "child_marriage" = "Value")

second_enrol <- read_csv("violence_factors/female_second_school_enrol.csv") %>% 
...
labour_part_rate <- read_csv("violence_factors/female_labour_force_part.csv") %>% 
  select("ref_area.label", "obs_value") %>% 
  rename("country" = "ref_area.label", "labour_part" = "obs_value")

poverty_rate_oecd <- read_csv("violence_factors/poverty_rate_oecd.csv") %>% 
  filter(SUBJECT == "TOT", TIME == 2019) %>% 
  select("LOCATION", "Value") %>% 
  rename("location" = "LOCATION", "poverty_rate_oecd" = "Value")

unemployment_rate_ILO <- read_csv("violence_factors/unemployment_rate_ILO.csv") %>% 
...
women_in_politics <- read_csv("violence_factors/women_in_politics.csv") %>% 
  filter(TIME == "2019", FREQUENCY == "A") %>% 
  select("LOCATION", "Value") %>% 
  rename("location" = "LOCATION", "women_in_politics" = "Value")

view(women_in_politics)
...
```
```{r}
ggplot(data=prev_viol_factors, mapping = aes(x = ATTITUDEVIOL, y = labour_part)) + 
  stat_smooth(method="lm")+
  geom_point()+
  labs (
    title="Prevalence of violence in the lifetime by Labour Participation",
