---
title: "Predicting Violence Against Women Using Women's Opportunities Index"
description: |
...
# Literature Review
There is an extensive literature on what regards violence against women. Since our research focuses on the Women Economic Opportunitie's index, and specifically, Labour Participation, Access to Finance, Women's Legal Status, and Education and Training.

Women are disadvantaged in entrepreneuship. An assumption that instituitions of entrepreneuship are genderless have resulted in structures that pretend to be "genderless" when they are not (Pathak et. al.). It is well-known that organizations, by not identifying or addressing the barriers that women might uniquely face, such as childrearing and domestic labour (Acker). Our research dialogues with previous literature in assering that difficulty in women's access to finance and labour participation further limit the possibility of firming women's independence from, at best, their domestic and childrearing duties, and at worse,  from their partners. By constraining women from participating in labout in equal and fair manners, as well accessing finance, we argue that this could be increasing violence against women. 

Women's education and legal status are essential features for their empowerment and reduction of dependency. 
...
```{r LAWDOMVIOLincome layout="l-body-outset"}
...
```{r BarStack ATTITUDEVIOLByIncomeGroup}
ggplot(violence_region_wide, 
       aes(x = ATTITUDEVIOL, 
           fill = IncomeGroup,
           )) + 
...
violence_region_coordinates_wide %>%
  ...
  geom_label(aes(x=23.3, y=19.6), label=outlier_low,fill = "white", fontface="bold",color="black", nudge_y=7, size=4, alpha=0.01)
```
...
corr_matrix <- round(
  cor(relevant_factors, use="complete.obs"),
  digits = 2 
)
...
p.mat <- cor.mtest(relevant_factors)

corrplot(corr_matrix, method="color", tl.col="black", tl.srt=45, type="upper", p.mat=p.mat, sig.level = 0.05, addCoef.col = "black",number.font= 55, diag=FALSE, insig = "blank")
```


# Regression Model

## Child Marriage Regression Model
...
## Labour Participation Rate Regression Model
```{r message=FALSE, warning=FALSE}
...
## Women's Opportunities Index Regression Model
...
## Women's Education Regression Model
```{r message=FALSE, warning=FALSE}
prev_opp_index <- brm(formula = prev_viol~education, data=relevant_factors,refresh=0)
...
PLOTTING PREVLIFTIME AND LABOUR POLICY AND PRACTICE, ACCESS TO FINANCE, EDUCATION AND TRAINING AND WOMENS LEGAL STATUS 
```{r}
